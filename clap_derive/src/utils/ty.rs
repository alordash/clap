...
use syn::{
    GenericArgument, Path, PathArguments, PathArguments::AngleBracketed, PathSegment,
...
impl Ty {
    pub(crate) fn from_syn_ty(ty: &Type) -> Sp<Self> {
        use self::Ty::{
            Option, OptionOption, OptionVec, OptionVecVec, Other, Unit, Vec, VecVec,
        };
...
pub(crate) fn inner_type(field_ty: &Type) -> &Type {
    ...
    match *ty {
        ...
        Ty::OptionVecVec => {
            sub_type(field_ty).and_then(sub_type).and_then(sub_type).unwrap_or(field_ty)
        }
        _ => field_ty,
    }
}
