...
pub(crate) fn derive_parser(input: &DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    let pkg_name_tokens = if pkg_name.is_empty() {
        ...
        quote!({ let _ = ::core::env!("CARGO_PKG_NAME"); # pkg_name })
    };
    match input.data {
...
fn gen_for_struct(
    ...
    Ok(
        quote! {
            ...
            clippy::redundant_locals,)] impl # impl_generics clap::Parser for # item_name
            # ty_generics # where_clause {} # into_app # args
        },
    )
}
#[rsubstitute::mock(base)]
fn gen_for_enum(
    ...
    Ok(
        quote! {
            #[automatically_derived] impl # impl_generics clap::Parser for # item_name #
            ty_generics # where_clause {} # into_app # subcommand
        },
    )
}
