...
pub(crate) fn gen_for_enum(
    ...
    if !matches!(&* item.kind(), Kind::Value) {
        abort! {
            item.kind().span(), "`{}` cannot be used with `#[value]`", item.kind()
            .name(),
        }
    }
    ...
    Ok(
        quote! {
            ...
            impl clap::ValueEnum for # item_name { # value_variants # to_possible_value }
        },
    )
}
#[cfg_attr(test, rsubstitute::mock(base))]
fn lits(variants: &[(&Variant, Item)]) -> Result<Vec<(TokenStream, Ident)>, syn::Error> {
    ...
    for (variant, item) in variants {
        ...
        genned
            .push((
                quote_spanned! {
                    variant.span() => { # deprecations
                    clap::builder::PossibleValue::new(# name) # fields }
                },
                variant.ident.clone(),
            ));
    }
...
fn gen_to_possible_value(item: &Item, lits: &[(TokenStream, Ident)]) -> TokenStream {
    ...
    let deprecations = item.deprecations();
    quote! {
        fn to_possible_value <'a > (& self) -> ::std::option::Option <
        clap::builder::PossibleValue > { # deprecations match self { # (Self::# variant
        => Some(# lit),) * _ => None } }
    }
}
