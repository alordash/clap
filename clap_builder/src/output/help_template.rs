...
impl<'cmd, 'writer> HelpTemplate<'cmd, 'writer> {
    ...
    pub(crate) fn new(
        ...
        debug!("HelpTemplate::new cmd={}, use_long={}", cmd.get_name(), use_long);
        ...
        for part in parts {
            if let Some((tag, rest)) = part.split_once('}') {
                match tag {
                    ...
                    "usage-heading" => {
                        let _ = write!(
                            self.writer, "{}Usage:{}", self.styles.get_usage().render(),
...
impl HelpTemplate<'_, '_> {
    ...
    fn write_before_help(&mut self) {
        ...
        let before_help = if self.use_long {
            self.cmd.get_before_long_help().or_else(|| self.cmd.get_before_help())
        } else {
    ...
    fn write_after_help(&mut self) {
        ...
        let after_help = if self.use_long {
            self.cmd.get_after_long_help().or_else(|| self.cmd.get_after_help())
        } else {
...
impl HelpTemplate<'_, '_> {
    ...
    fn write_args(&mut self, args: &[&Arg], _category: &str, sort_key: ArgSortKey) {
        ...
        for &arg in args.iter().filter(|arg| { should_show_arg(self.use_long, arg) }) {
            ...
            debug!(
                "HelpTemplate::write_args: arg={:?} longest={}", arg.get_id(), longest
            );
    ...
    fn write_arg(&mut self, arg: &Arg, next_line_help: bool, longest: usize) {
        ...
        self.short(arg);
        self.long(arg);
        self.writer.push_styled(&arg.stylize_arg_suffix(self.styles, None));
        self.align_to_about(arg, next_line_help, longest);
        let about = if self.use_long {
            arg.get_long_help().or_else(|| arg.get_help()).unwrap_or_default()
        } else {
            arg.get_help().or_else(|| arg.get_long_help()).unwrap_or_default()
        };
    ...
    fn align_to_about(&mut self, arg: &Arg, next_line_help: bool, longest: usize) {
        debug!(
            "HelpTemplate::align_to_about: arg={}, next_line_help={}, longest={}", arg
            .get_id(), next_line_help, longest
        );
    ...
    fn help(
        ...
        debug!(
            "HelpTemplate::help: help_width={}, spaces={}, avail={}", spaces, help
            .display_width(), avail_chars
        );
        ...
        if let Some(arg) = arg {
            if !arg.is_hide_possible_values_set() && self.use_long_pv(arg) {
                ...
                if !possible_vals.is_empty() {
                    debug!(
                        "HelpTemplate::help: Found possible vals...{possible_vals:?}"
                    );
    ...
    fn arg_next_line_help(&self, arg: &Arg, spec_vals: &str, longest: usize) -> bool {
        if self.next_line_help || arg.is_next_line_help_set() || self.use_long {
            ...
            let h = arg.get_help().or_else(|| arg.get_long_help()).unwrap_or_default();
            let h_w = h.display_width() + display_width(spec_vals);
            let taken = longest + TAB_WIDTH * 2;
            self.term_w >= taken && (taken as f32 / self.term_w as f32) > 0.40
    ...
    fn spec_vals(&self, a: &Arg) -> String {
        ...
        if let Some(ref env) = a.env {
            if !a.is_hide_env_set() {
                ...
                let env_val = if !a.is_hide_env_values_set() {
                    format!(
                        "={}", env.1.as_ref().map(| s | s.to_string_lossy())
                        .unwrap_or_default()
                    )
                } else {
                    Default::default()
                ...
                let env_info = format!(
                    "{ctx}[env: {ctx:#}{ctx_val}{}{}{ctx_val:#}{ctx}]{ctx:#}", env.0
                    .to_string_lossy(), env_val
                );
                spec_vals.push(env_info);
            }
        }
        if a.is_takes_value_set() && !a.is_hide_default_value_set()
            && !a.default_vals.is_empty()
        {
            debug!(
                "HelpTemplate::spec_vals: Found default value...[{:?}]", a.default_vals
            );
            let dvs = a
                .default_vals
                .iter()
        ...
        let short_als = a
            ...
            .iter()
            .filter(|&als| als.1)
            .map(|als| format!("{ctx_val}-{}{ctx_val:#}", als.0));
        debug!("HelpTemplate::spec_vals: Found short aliases...{:?}", a.short_aliases);
        ...
        if !a.is_hide_possible_values_set() && !self.use_long_pv(a) {
            ...
            if !possible_vals.is_empty() {
                debug!(
                    "HelpTemplate::spec_vals: Found possible vals...{possible_vals:?}"
                );
                let pvs = possible_vals
                    ...
                    .join(&val_sep);
                spec_vals
                    .push(format!("{ctx}[possible values: {ctx:#}{pvs}{ctx}]{ctx:#}"));
            }
        }
    ...
    fn use_long_pv(&self, arg: &Arg) -> bool {
        self.use_long
            && arg.get_possible_values().iter().any(PossibleValue::should_show_help)
    }
}
#[cfg_attr(test, rsubstitute::mock(base))]
/// Subcommand handling
impl HelpTemplate<'_, '_> {
    ...
    fn write_flat_subcommands(&mut self, cmd: &Command, first: &mut bool) {
        debug!(
            "HelpTemplate::write_flat_subcommands, cmd={}, first={}", cmd.get_name(), *
            first
        );
        use std::fmt::Write as _;
        let header = &self.styles.get_header();
    ...
    fn write_subcommand(
        ...
        let about = cmd.get_about().or_else(|| cmd.get_long_about()).unwrap_or_default();
    ...
    fn sc_spec_vals(&self, a: &Command) -> String {
        ...
        let als = a.get_visible_aliases().map(|s| format!("{ctx_val}{s}{ctx_val:#}"));
        short_als.extend(als);
        let all_als = short_als.join(&val_sep);
        if !all_als.is_empty() {
            debug!(
                "HelpTemplate::spec_vals: Found aliases...{:?}", a.get_all_aliases()
                .collect::< Vec < _ >> ()
            );
            debug!(
                "HelpTemplate::spec_vals: Found short flag aliases...{:?}", a
                .get_all_short_flag_aliases().collect::< Vec < _ >> ()
            );
            debug!(
                "HelpTemplate::spec_vals: Found long flag aliases...{:?}", a
                .get_all_long_flag_aliases().collect::< Vec < _ >> ()
            );
            let plural = pluralize(short_als.len(), "", "es");
            spec_vals
                .push(format!("{ctx}[alias{plural}: {ctx:#}{all_als}{ctx}]{ctx:#}"));
        }
        spec_vals.join(" ")
    }
    fn subcommand_next_line_help(
        ...
        if self.next_line_help {
            ...
            let h = cmd.get_about().or_else(|| cmd.get_long_about()).unwrap_or_default();
            let h_w = h.display_width() + display_width(spec_vals);
            let taken = longest + TAB_WIDTH * 2;
            self.term_w >= taken && (taken as f32 / self.term_w as f32) > 0.40
                && h_w > (self.term_w - taken)
...
pub(crate) fn dimensions() -> (Option<usize>, Option<usize>) {
    #[cfg(not(feature = "wrap_help"))] return (None, None);
...
fn parse_env(var: &str) -> Option<usize> {
    some!(some!(std::env::var_os(var)) .to_str()).parse::<usize>().ok()
}
#[cfg_attr(test, rsubstitute::mock(base))]
fn should_show_arg(use_long: bool, arg: &Arg) -> bool {
    debug!("should_show_arg: use_long={:?}, arg={}", use_long, arg.get_id());
    if arg.is_hide_set() {
        return false;
    }
