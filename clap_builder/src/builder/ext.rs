...
impl Extensions {
    ...
    pub(crate) fn get<T: Extension>(&self) -> Option<&T> {
        ...
        self.extensions
            .get(&id)
            .map(|e| {
                e.downcast_ref::<T>().expect("`Extensions` tracks values by type")
            })
    }
    #[allow(dead_code)]
    ...
    pub(crate) fn remove<T: Extension>(&mut self) -> Option<T> {
        ...
        self.extensions
            .remove(&id)
            .map(|e| {
                e.downcast_into::<T>().expect("`Extensions` tracks values by type")
            })
    }
...
where
    T: std::fmt::Debug + Clone + std::any::Any + Send + Sync + 'static,
{}
