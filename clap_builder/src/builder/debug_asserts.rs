...
pub(crate) fn assert_app(cmd: &Command) {
    ...
    for arg in cmd.get_arguments() {
        ...
        assert!(
            ...
            "Command {}: Arguments like {} cannot be set on a multicall command", cmd
            .get_name(), arg.get_id()
        );
        ...
        for (short_alias, _) in &arg.short_aliases {
            short_flags
                .push(Flag::Arg(format!("-{short_alias}"), arg.get_id().as_str()));
        }
        ...
        if let Some(idx) = arg.index {
            if let Some((first, second)) = cmd
                .two_args_of(|x| x.is_positional() && x.get_index() == Some(idx))
            {
                panic!(
        ...
        for (_predicate, req_id) in &arg.requires {
            assert!(
                & arg.id != req_id, "Argument {} cannot require itself", arg.get_id()
            );
            assert!(
        ...
        for req in &arg.r_ifs {
            assert!(
                ! arg.is_required_set(),
                "Argument {}: `required` conflicts with `required_if_eq*`", arg.get_id()
            );
            assert!(
                cmd.id_exists(& req.0),
        ...
        for req in &arg.r_ifs_all {
            assert!(
                ! arg.is_required_set(),
                "Argument {}: `required` conflicts with `required_if_eq_all`", arg
                .get_id()
            );
            assert!(
                cmd.id_exists(& req.0),
        ...
        for req in &arg.r_unless {
            assert!(
                ! arg.is_required_set(),
                "Argument {}: `required` conflicts with `required_unless*`", arg.get_id()
            );
            assert!(
                cmd.id_exists(req),
    ...
    for group in cmd.get_groups() {
        ...
        for arg in &group.requires {
            assert!(
                cmd.id_exists(arg),
                "Command {}: Argument group '{}' requires non-existent '{}' id", cmd
                .get_name(), group.get_id(), arg
            );
        }
    ...
    for sc in cmd.get_subcommands() {
        assert!(
            subs.insert(sc.get_name()), "Command {}: command name `{}` is duplicated",
            cmd.get_name(), sc.get_name()
        );
        for alias in sc.get_all_aliases() {
            assert!(
                subs.insert(alias), "Command {}: command `{}` alias `{}` is duplicated",
    ...
    if let Some(help_template) = cmd.get_help_template() {
        assert!(
            ! help_template.to_string().contains("{flags}"), "Command {}: {}", cmd
            .get_name(),
            "`{flags}` template variable was removed in clap3, they are now included in `{options}`",
        );
        assert!(
            ! help_template.to_string().contains("{unified}"), "Command {}: {}", cmd
            .get_name(),
            "`{unified}` template variable was removed in clap3, use `{options}` instead"
        );
        #[cfg(feature = "unstable-v5")]
        assert!(
            ! help_template.to_string().contains("{bin}"), "Command {}: {}", cmd
            .get_name(),
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        match (self, other) {
            ...
            | (Flag::Arg(s1, _), Flag::Command(s2, _)) => {
                if s1 == s2 { Ordering::Equal } else { s1.cmp(s2) }
            }
        }
    }
}
...
fn assert_app_flags(cmd: &Command) {
    macro_rules! checker {
        ($a:ident conflicts $($b:ident)|+) => {
            if cmd.$a () { let mut s = String::new(); $(if cmd.$b () { use
            std::fmt::Write; write!(& mut s,
            "  AppSettings::{} conflicts with AppSettings::{}.\n", std::stringify!($b),
            std::stringify!($a)) .unwrap(); })+ if ! s.is_empty() { panic!("{}\n{}", cmd
            .get_name(), s) } }
        };
    }
...
fn _verify_positionals(cmd: &Command) -> bool {
    ...
    let highest_idx = cmd
        .get_keymap()
        .keys()
        .filter_map(|x| { if let KeyType::Position(n) = x { Some(*n) } else { None } })
        .max()
        .unwrap_or(0);
    ...
    for arg in cmd.get_arguments() {
        if arg.index.unwrap_or(0) == highest_idx {
            ...
            if arg.is_trailing_var_arg_set() {
                assert!(
                    arg.is_multiple(),
                    "{}:{}: `Arg::trailing_var_arg` must accept multiple values", cmd
                    .get_name(), arg.get_id()
                );
            }
        } else {
            assert!(
                ...
                "{}:{}: `Arg::trailing_var_arg` can only apply to last positional", cmd
                .get_name(), arg.get_id()
            );
        }
    }
    let only_highest = |a: &Arg| {
        a.is_multiple() && (a.get_index().unwrap_or(0) != highest_idx)
    };
    if cmd.get_positionals().any(only_highest) {
        ...
        let count = cmd
            .get_positionals()
            .filter(|p| {
                p.is_multiple_values_set() && p.get_value_terminator().is_none()
                    && !p.get_num_args().expect(INTERNAL_ERROR_MSG).is_fixed()
            })
            .count();
        let ok = count <= 1
            || (last.is_last_set() && last.is_multiple() && second_to_last.is_multiple()
                && count == 2);
        assert!(
            ok,
    ...
    if cmd.get_positionals().any(|p| p.is_last_set() && p.is_required_set())
        && cmd.has_subcommands() && !cmd.is_subcommand_negates_reqs_set()
    {
        panic!(
...
fn assert_arg(arg: &Arg) {
    ...
    assert!(
        ...
        "Argument '{}' cannot conflict with itself", arg.get_id(),
    );
    assert!(
        arg.get_num_args().unwrap_or(1.into()).max_values() <= arg.get_action()
        .max_num_args().max_values(),
        "Argument `{}`'s action {:?} is incompatible with `num_args({:?})`", arg
        .get_id(), arg.get_action(), arg.get_num_args().unwrap_or(1.into())
    );
    if let Some(action_type_id) = arg.get_action().value_type_id() {
        assert_eq!(
            action_type_id, arg.get_value_parser().type_id(),
            "Argument `{}`'s selected action {:?} contradicts `value_parser` ({:?})", arg
            .get_id(), arg.get_action(), arg.get_value_parser()
        );
    }
    if arg.get_value_hint() != ValueHint::Unknown {
        assert!(
            arg.is_takes_value_set(), "Argument '{}' has value hint but takes no value",
            arg.get_id()
        );
        if arg.get_value_hint() == ValueHint::CommandWithArguments {
            assert!(
    ...
    if arg.index.is_some() {
        ...
        assert!(
            ...
            arg.get_id(), arg.get_action(), if arg.get_id() == Id::HELP {
            " (`mut_arg` no longer works with implicit `--help`)" } else if arg.get_id()
            == Id::VERSION { " (`mut_arg` no longer works with implicit `--version`)" }
            else { "" }
        );
    }
    ...
    if num_vals != ValueRange::EMPTY {
        ...
        if num_vals.max_values() < num_val_names {
            panic!(
                "Argument {}: Too many value names ({}) compared to `num_args` ({})", arg
                .get_id(), num_val_names, num_vals
            );
        }
    }
    assert_eq!(
        num_vals.is_multiple(), arg.is_multiple_values_set(),
        "Argument {}: mismatch between `num_args` ({}) and `multiple_values`", arg
        .get_id(), num_vals,
    );
...
fn assert_arg_flags(arg: &Arg) {
    macro_rules! checker {
        ($a:ident requires $($b:ident)|+) => {
            if arg.$a () { let mut s = String::new(); $(if ! arg.$b () { use
            std::fmt::Write; write!(& mut s,
            "  Arg::{} is required when Arg::{} is set.\n", std::stringify!($b),
            std::stringify!($a)) .unwrap(); })+ if ! s.is_empty() {
            panic!("Argument {:?}\n{}", arg.get_id(), s) } }
        };
    }
