        ...
        if confidence > 0.7 {
            ...
            let pos = candidates
                .binary_search_by(|probe| {
                    if probe.0 > confidence { Ordering::Greater } else { Ordering::Less }
                })
                .unwrap_or_else(|e| e);
        ...
        None => {
            subcommands
                ...
                .filter_map(|subcommand| {
                    ...
                    let score = some!(
                        remaining_args.iter().position(| x | subcommand_name == * x)
                    );
...
mod test {
    ...
    fn best_fit() {
        ...
        assert_eq!(
            did_you_mean("alignmentScorr", p_vals.iter()), vec!["alignmentStart",
            "alignmentScore"]
        );
    }
    #[cfg_attr(test, rsubstitute::mock(base))]
    ...
    fn flag_missing_letter() {
        let p_vals = ["test", "possible", "values"];
        assert_eq!(
            did_you_mean_flag("tst", & [], p_vals.iter(), []), Some(("test".to_owned(),
            None))
        );
    }
    #[cfg_attr(test, rsubstitute::mock(base))]
    ...
    fn flag_ambiguous() {
        let p_vals = ["test", "temp", "possible", "values"];
        assert_eq!(
            did_you_mean_flag("te", & [], p_vals.iter(), []), Some(("temp".to_owned(),
            None))
        );
    }
    #[cfg_attr(test, rsubstitute::mock(base))]
