use std::{cell::Cell, ffi::{OsStr, OsString}};
...
impl<'cmd> Parser<'cmd> {
    ...
    pub(crate) fn get_matches_with(
        ...
        ok!(
            self.parse(matcher, raw_args, args_cursor).inspect_err(| _err | { if self.cmd
            .is_ignore_errors_set() { #[cfg(feature = "env")] let _ = self
            .add_env(matcher); let _ = self.add_defaults(matcher); } })
        );
    ...
    pub(crate) fn parse(
        ...
        while let Some(arg_os) = raw_args.next(&mut args_cursor) {
            debug!(
                "Parser::get_matches_with: Begin parsing '{:?}'", arg_os.to_value_os(),
            );
            if !trailing_values {
                    ...
                    || !matches!(parse_state, ParseState::Opt(_) | ParseState::Pos(_))
                {
                    ...
                    if let Some(sc_name) = sc_name {
                        ...
                        {
                            ok!(
                                self.parse_help_subcommand(raw_args.remaining(& mut
                                args_cursor))
                            );
                ...
                if arg_os.is_escape() {
                    if matches!(
                        & parse_state, ParseState::Opt(opt) | ParseState::Pos(opt) if
                    ...
                    let parse_result = ok!(
                        self.parse_long_arg(matcher, long_arg, long_value, & parse_state,
                        pos_counter, & mut valid_arg_found,)
                    );
                    debug!(
                        "Parser::get_matches_with: After parse_long_arg {parse_result:?}"
                    );
                    match parse_result {
                        ParseResult::NoArg => {
                    ...
                    let parse_result = ok!(
                        self.parse_short_arg(matcher, short_arg, & parse_state,
                        pos_counter, & mut valid_arg_found,)
                    );
                    debug!(
                        "Parser::get_matches_with: After parse_short_arg {parse_result:?}"
                    );
                    match parse_result {
                        ParseResult::NoArg => {}
                        ParseResult::ValuesDone => {
                ...
                if let ParseState::Opt(id) = &parse_state {
                    let arg = &self.cmd[id];
                    let parse_result = if let Some(parse_result) = self
                        .check_terminator(arg, arg_os.to_value_os())
                    {
                        parse_result
                    } else {
                        let trailing_values = false;
                        let arg_values = matcher
                            .pending_values_mut(id, None, trailing_values);
            ...
            pos_counter = {
                ...
                let missing_pos = self.cmd.is_allow_missing_positional_set()
                    && is_second_to_last && !trailing_values;
            ...
            if let Some(arg) = self.cmd.get_keymap().get(&pos_counter) {
                ...
                if matcher.pending_arg_id() != Some(arg.get_id())
                    || !arg.is_multiple_values_set()
                {
                    ok!(self.resolve_pending(matcher));
                }
                parse_state = if let Some(parse_result) = self
                    .check_terminator(arg, arg_os.to_value_os())
                {
            ...
            } else if let Some(external_parser) = self
                .cmd
                ...
                .cloned()
            {
                ...
                for raw_val in raw_args.remaining(&mut args_cursor) {
                    let val = ok!(
                        external_parser.parse_ref(self.cmd, None, raw_val,
        ...
        if let Some(ref pos_sc_name) = subcmd_name {
            ...
            let sc_name = self
                ...
                .to_owned();
            ok!(
                self.parse_subcommand(& sc_name, matcher, raw_args, args_cursor,
                keep_state)
            );
        }
        Ok(())
    ...
    fn match_arg_error(
        ...
        if trailing_values {
            if self.possible_subcommand(arg_os.to_value(), valid_arg_found).is_some() {
        ...
        let suggested_trailing_arg = !trailing_values && self.cmd.has_positionals()
    ...
    fn is_new_arg(
        &self,
        next: &clap_lex::ParsedArg<'_>,
        current_positional: &Arg,
    ) -> bool {
        #![allow(clippy::needless_bool)]
        debug!(
            "Parser::is_new_arg: {:?}:{}", next.to_value_os(), current_positional
            .get_id()
        );
    ...
    fn parse_subcommand(
        ...
        if let Some(sc) = self.cmd._build_subcommand(sc_name) {
            ...
            debug!("Parser::parse_subcommand: About to parse sc={}", sc.get_name());
            {
                ...
                if let Err(error) = p
                    .get_matches_with(&mut sc_matcher, raw_args, args_cursor)
                {
    ...
    fn parse_long_arg(
        ...
        if matches!(
            parse_state, ParseState::Opt(opt) | ParseState::Pos(opt) if self.cmd[opt]
        ...
        if let Some((_long_arg, arg)) = arg {
            ...
            *valid_arg_found = true;
            if arg.is_takes_value_set() {
                ...
                debug!(
                    "Parser::parse_long_arg({long_arg:?}): Got invalid literal `{rest:?}`"
                );
        ...
        {
            debug!(
                "Parser::parse_long_args: positional at {pos_counter} allows hyphens"
            );
    ...
    fn parse_short_arg(
        ...
        if matches!(
            parse_state, ParseState::Opt(opt) | ParseState::Pos(opt) if self.cmd[opt]
            .is_allow_hyphen_values_set() || (self.cmd[opt]
            .is_allow_negative_numbers_set() && short_arg.is_negative_number())
        ) {
            ...
            .unwrap_or_default() && short_arg.is_negative_number()
        {
        ...
        {
            debug!(
                "Parser::parse_short_args: positional at {pos_counter} allows hyphens"
            );
            return Ok(ParseResult::MaybeHyphenValue);
        }
        ...
        debug_assert_eq!(
            res, Ok(()), "tracking of `flag_subcmd_skip` is off for `{short_arg:?}`"
        );
            ...
            if let Some(arg) = self.cmd.get_keymap().get(&c) {
                ...
                *valid_arg_found = true;
                if !arg.is_takes_value_set() {
                    ...
                    let trailing_idx = None;
                    ret = ok!(
                        self.react(Some(ident), ValueSource::CommandLine, arg,
                        arg_values, trailing_idx, matcher,)
                    );
                    continue;
                }
                let val = short_arg.clone().next_value_os().unwrap_or_default();
                debug!(
                    "Parser::parse_short_arg:iter:{c}: val={val:?}, short_arg={short_arg:?}"
                );
                let val = Some(val).filter(|v| !v.is_empty());
                let (val, has_eq) = if let Some(val) = val
    ...
    fn parse_opt_value(
        ...
        debug!(
            "Parser::parse_opt_value; arg={}, val={:?}, has_eq={:?}", arg.get_id(),
            attached_value, has_eq
        );
        ...
        if arg.is_require_equals_set() && !has_eq {
            if arg.get_min_vals() == 0 {
                ...
                let trailing_idx = None;
                let react_result = ok!(
                    self.react(Some(ident), ValueSource::CommandLine, arg, arg_values,
    ...
    fn push_arg_values(
        ...
        for raw_val in raw_vals {
            ...
            let val = ok!(
                value_parser.parse_ref(self.cmd, Some(arg), & raw_val, source)
            );
    ...
    fn resolve_pending(&self, matcher: &mut ArgMatcher) -> ClapResult<()> {
        ...
        let _ = ok!(
            self.react(pending.ident, ValueSource::CommandLine, arg, pending.raw_vals,
            pending.trailing_idx, matcher,)
        );
        Ok(())
    }
    fn react(
        ...
        ok!(self.resolve_pending(matcher));
        debug!(
            "Parser::react action={:?}, identifier={:?}, source={:?}", arg.get_action(),
            ident, source
        );
        ...
        if raw_vals.is_empty() {
            if !arg.default_missing_vals.is_empty() {
                ...
                raw_vals
                    .extend(
                        arg.default_missing_vals.iter().map(|s| s.as_os_str().to_owned()),
                    );
            }
        }
            ...
            {} else {
                ...
                for (i, raw_val) in raw_vals.into_iter().enumerate() {
                            ...
                            && trailing_idx == Some(i))
                    {
                        ...
                        split_raw_vals
                            .extend(raw_val.split(val_delim).map(|x| x.to_owned()));
                    }
                }
        ...
        match arg.get_action() {
            ArgAction::Set => {
                ...
                if matcher.remove(arg.get_id())
                    && !(self.cmd.is_args_override_self()
                        || arg.overrides.contains(arg.get_id()))
                {
                    return Err(
                        ClapError::argument_conflict(
                            self.cmd,
                            arg.to_string(),
                            vec![arg.to_string()],
                            Usage::new(self.cmd).create_usage_with_title(&[]),
    ...
    fn verify_num_args(&self, arg: &Arg, raw_vals: &[OsString]) -> ClapResult<()> {
        ...
        if 0 < expected.min_values() && actual == 0 {
            ...
            if expected != actual {
                ...
                return Err(
                    ClapError::wrong_number_of_values(
                        self.cmd,
                        arg.to_string(),
                        expected,
                        actual,
    ...
    fn add_env(&mut self, matcher: &mut ArgMatcher) -> ClapResult<()> {
        ...
        for arg in self.cmd.get_arguments() {
            ...
            if let Some((_, Some(ref val))) = arg.env {
                ...
                let trailing_idx = None;
                let _ = ok!(
                    self.react(None, ValueSource::EnvVariable, arg, arg_values,
                    trailing_idx, matcher,)
                );
            }
        }
    ...
    fn add_default_value(&self, arg: &Arg, matcher: &mut ArgMatcher) -> ClapResult<()> {
                ...
                for (id, val, default) in arg.default_vals_ifs.iter() {
                    ...
                    if add {
                        if let Some(default) = default {
                            let arg_values = default
                                .iter()
                                .map(|os_str| os_str.to_os_string())
                                .collect();
                            let trailing_idx = None;
                            let _ = ok!(
                                self.react(None, ValueSource::DefaultValue, arg, arg_values,
                                trailing_idx, matcher,)
                            );
                        }
                        return Ok(());
        ...
        if !arg.default_vals.is_empty() {
            debug!("Parser::add_default_value:iter:{}: has default vals", arg.get_id());
            if matcher.contains(arg.get_id()) {
                debug!("Parser::add_default_value:iter:{}: was used", arg.get_id());
            } else {
                debug!("Parser::add_default_value:iter:{}: wasn't used", arg.get_id());
            ...
            debug!(
                "Parser::add_default_value:iter:{}: doesn't have default vals", arg
                .get_id()
            );
        }
        Ok(())
...
impl Parser<'_> {
    ...
    fn did_you_mean_error(
        ...
        ClapError::unknown_argument(
            ...
            did_you_mean,
            suggested_trailing_arg,
            Usage::new(self.cmd).required(&required).create_usage_with_title(&used),
        )
    }
...
enum ParseResult {
    ...
    UnneededAttachedValue { rest: String, used: Vec<Id>, arg: String },
    /// This flag might be an hyphen Value.
    MaybeHyphenValue,
    /// Equals required but not provided.
    EqualsNotProvided { arg: String },
    /// Failed to match a Arg.
    NoMatchingArg { arg: String },
