...
impl<'cmd> Validator<'cmd> {
    ...
    fn validate_exclusive(&self, matcher: &ArgMatcher) -> ClapResult<()> {
        ...
        matcher
            ...
            .find_map(|(id, _)| {
                ...
                self.cmd.find(id).filter(|&arg| arg.is_exclusive_set() && args_count > 1)
            })
            .map(|arg| {
                Err(
    ...
    fn build_conflict_err(
        ...
        Err(Error::argument_conflict(self.cmd, former_arg.to_string(), conflicts, usg))
    }
    fn build_conflict_err_usage(
        ...
        let used_filtered: Vec<Id> = matcher
            ...
            .map(|(n, _)| n)
            .filter(|n| {
                self.cmd.find(n).map(|a| !a.is_hide_set()).unwrap_or_default()
            })
        ...
        let required: Vec<Id> = used_filtered
            ...
            .filter(|key| {
                !used_filtered.contains(key) && !conflicting_keys.contains(key)
            })
    ...
    fn validate_required(
        ...
        let is_exclusive_present = matcher
            ...
            .filter(|(_, matched)| matched.check_explicit(&ArgPredicate::IsPresent))
            .any(|(id, _)| {
                self.cmd.find(id).map(|arg| arg.is_exclusive_set()).unwrap_or_default()
            });
        debug!(
            "Validator::validate_required: is_exclusive_present={is_exclusive_present}"
        );
            ...
            .filter(|r| !matcher.check_explicit(r, &ArgPredicate::IsPresent))
        {
            ...
            if let Some(arg) = self.cmd.find(arg_or_group) {
                ...
                {
                    debug!(
                        "Validator::validate_required:iter: Missing {:?}", arg.get_id()
                    );
                ...
                {
                    debug!(
                        "Validator::validate_required:iter: Missing {:?}", group.get_id()
                    );
            ...
            .filter(|a| !matcher.check_explicit(a.get_id(), &ArgPredicate::IsPresent))
        {
            ...
            for (other, val) in &a.r_ifs {
                if matcher.check_explicit(other, &ArgPredicate::Equals(val.into())) {
                    debug!(
                        "Validator::validate_required:iter: Missing {:?}", a.get_id()
                    );
                    required = true;
                }
            }
            ...
            if match_all && !a.r_ifs_all.is_empty() {
                debug!("Validator::validate_required:iter: Missing {:?}", a.get_id());
                required = true;
            }
        ...
        if !self.cmd.is_allow_missing_positional_set() {
            for pos in self
                ...
                .get_positionals()
                .filter(|a| {
                    !matcher.check_explicit(a.get_id(), &ArgPredicate::IsPresent)
                })
            {
                if pos.get_index() < Some(highest_index) {
                    debug!(
                        "Validator::validate_required:iter: Missing {:?}", pos.get_id()
                    );
...
impl Conflicts {
    ...
    fn gather_conflicts(&self, cmd: &Command, arg_id: &Id) -> Vec<Id> {
        ...
        let arg_id_conflicts = if let Some(arg_id_conflicts) = self
            .get_direct_conflicts(arg_id)
        {
            arg_id_conflicts
        } else {
