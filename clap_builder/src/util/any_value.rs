...
impl AnyValue {
    pub(crate) fn new<V: std::any::Any + Clone + Send + Sync + 'static>(
        inner: V,
    ) -> Self {
    ...
    pub(crate) fn downcast_into<T: std::any::Any + Clone + Send + Sync>(
        self,
    ) -> Result<T, Self> {
        let id = self.id;
        let value = ok!(
            std::sync::Arc::downcast::< T > (self.inner).map_err(| inner | Self { inner,
            id })
        );
...
impl std::fmt::Debug for AnyValueId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        #[cfg(not(debug_assertions))] { self.type_id.fmt(f) }
        #[cfg(debug_assertions)] { f.debug_struct(self.type_name).finish() }
    }
}
#[cfg_attr(test, rsubstitute::mock(base))]
